...
const T_IMPACT = 400; // choque: flash + sacudida
...
export default function SplashScreen() {
  ...
  useEffect(() => {
    ...
    const timers = [
      setTimeout(() => setPhase('settled'), T_ENTER),
      setTimeout(() => setPhase('impact'), T_ENTER + T_SETTLE),
      setTimeout(() => setPhase('hold'), T_ENTER + T_SETTLE + T_IMPACT),
      setTimeout(
        () => setVisible(false),
        ...
        >
          ...
          {!reduceMotion && (
            <motion.div
              ...
              initial={{ opacity: 0 }}
              animate={{ opacity: impacting ? [0, 0.5, 0] : 0 }}
